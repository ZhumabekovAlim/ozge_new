...
func (r *ContractRepository) GetByToken(token string) (*models.Contract, error) {
	query := `SELECT contracts.id, company_id, template_id, contract_token, iin, fio, serial_number, generated_file_path, client_filled, method, company_sign, created_at, companies.name, companies.iin, companies.phone FROM contracts
